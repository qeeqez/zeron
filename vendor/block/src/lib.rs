...
// Vendored fix (upstream `block` 0.1.6 is unmaintained): `enum Class {}` is
// uninhabited, so `static _NSConcreteStackBlock: Class` trips rustc's
// `uninhabited_static` future-incompatibility lint (rust-lang/rust#74840).
// An inhabited opaque struct is ABI-identical — `Class` is only ever used
// behind pointers. Same approach as the maintained `block2` crate.
#[repr(C)]
struct Class {
    _priv: [*mut c_void; 32],
}
