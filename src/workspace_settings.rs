...
impl Workspace {
    ...
    fn save_settings_inner(&mut self, force_theme: bool) {
        ...
        crate::persist::save_settings(&crate::persist::Settings {
            ...
            notify_background: self.notify_background,
    ...
    /// clamped to the role's bounds — the single write path for the
    /// Appearance steppers and the Cmd-=/Cmd--/Cmd-0 zoom shortcuts.
    /// Persists, re-applies the theme and remeasures the transcript.
    pub(crate) fn set_font_size(&mut self, next: f32, code: bool, window: &mut Window, cx: &mut Context<Self>) {
        use crate::appearance::{CODE_FONT_SIZE_MAX, FONT_SIZE_MAX, FONT_SIZE_MIN};
        let next = next.clamp(FONT_SIZE_MIN, if code { CODE_FONT_SIZE_MAX } else { FONT_SIZE_MAX });
        if code {
    ...
    /// Cmd-=/Cmd-+ and Cmd--: step the interface font size (the rem base) by
    /// `delta` px, clamped to the shared bounds.
    pub(crate) fn zoom_font(&mut self, delta: f32, window: &mut Window, cx: &mut Context<Self>) {
        self.set_font_size(self.font_size + delta, false, window, cx);
    }

    /// Cmd-0: restore the default interface font size.
    pub(crate) fn reset_font_zoom(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        self.set_font_size(crate::appearance::FONT_SIZE_DEFAULT, false, window, cx);
    }
