...
pub static SHORTCUT_SPECS: &[ShortcutSpec] = &[
    ...
    spec!("cmd-=", ZoomIn, "Zoom in (larger text)", General),
    spec!("cmd--", ZoomOut, "Zoom out (smaller text)", General),
    spec!("cmd-0", ZoomReset, "Reset text size", General),
    ...
    spec!("cmd-p", GoToFile, "Go to file", Navigation),
